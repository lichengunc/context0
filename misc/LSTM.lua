...
function LSTM.lstm(input_size_v, input_size_w, output_size, rnn_size, dropout)
  ...
  if dropout > 0 then 
    -- xv = nn.Dropout(dropout)(xv) -- apply dropout on visual-embedding
    xw = nn.Dropout(dropout)(xw) -- apply dropout on word-embedding
  end 

  -- evaluate the input sums at once for efficienty
